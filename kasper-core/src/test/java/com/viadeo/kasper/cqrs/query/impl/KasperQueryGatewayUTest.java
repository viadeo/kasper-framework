...
public class KasperQueryGatewayUTest {
    ...
    private final InterceptorChainRepository interceptorChainRepository;
    ...
    public KasperQueryGatewayUTest() {
        ...
        queryGateway = new KasperQueryGateway(queryHandlersLocator, interceptorChainRepository);
    }

    @After
    public void clean() {
        reset(queryHandlersLocator, interceptorChainRepository);
    }
    ...
    public void getInterceptorChain_withUnknownQuery_shouldReturnNoChain() {
        // Given
        when(interceptorChainRepository.get(any(Class.class))).thenReturn(Optional.absent());
        ...
        // When
        Optional<InterceptorChain<Query,QueryResponse<QueryResult>>> requestActorChain = queryGateway.getInterceptorChain(Query.class);
    ...
    public void getInterceptorChain_withQueryHandler_shouldBeOk() {
        // Given
        QueryHandler<Query,QueryResult> queryHandler = new QueryHandlerWithFiltersForTest();

        KasperQueryGateway queryGateway = new KasperQueryGateway();
        queryGateway.register(queryHandler);

        // When
        Optional<InterceptorChain<Query, QueryResponse<QueryResult>>> interceptorChain = queryGateway.getInterceptorChain(Query.class);
    ...
    public void getInterceptorChain_withQueryHandler_withCache_shouldBeOk() {
        // Given
        QueryHandler<Query,QueryResult> queryHandler = new QueryHandlerWithCacheForTest();
