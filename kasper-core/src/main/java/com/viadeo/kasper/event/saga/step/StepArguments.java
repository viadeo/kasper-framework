...
public class StepArguments {
    ...
    public StepArguments(Method method) {
        ...
        if (method.getParameterTypes().length == 1) {
            ...
            if ((Event.class.isAssignableFrom(parameterTypes[0]) && Context.class.isAssignableFrom(parameterTypes[1])) ||
                ...
                for (int i = 0; i < parameterTypes.length; i++) {
                    this.parameterTypes.put(parameterTypes[i], i);
                }
    ...
    private EventMessage createEventMessage(final Context context, Event event) {
        return new EventMessage(
                new GenericEventMessage(event,
