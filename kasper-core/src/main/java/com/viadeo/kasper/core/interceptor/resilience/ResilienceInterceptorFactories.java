...
public class ResilienceInterceptorFactories {
    ...
    public static InterceptorFactory<Command, CommandResponse> forCommand(final MetricRegistry metricRegistry, final ResiliencePolicy policy) {
        return new InterceptorFactory<Command,CommandResponse>() {
            ...
            public Optional<InterceptorChain<Command, CommandResponse>> create(final TypeToken<?> type) {
                ...
                return Optional.of(InterceptorChain.makeChain(
                        new ResilienceInterceptor<Command,CommandResponse>(metricRegistry, policy) {
                            @Override
    ...
    public static InterceptorFactory<Query,QueryResponse<QueryResult>> forQuery(final MetricRegistry metricRegistry, final ResiliencePolicy policy) {
        return new InterceptorFactory<Query,QueryResponse<QueryResult>>() {
            ...
            public Optional<InterceptorChain<Query, QueryResponse<QueryResult>>> create(final TypeToken<?> type) {
                checkNotNull(type);
                return Optional.of(InterceptorChain.makeChain(
                    new ResilienceInterceptor<Query,QueryResponse<QueryResult>>(metricRegistry, policy) {
                        @Override
