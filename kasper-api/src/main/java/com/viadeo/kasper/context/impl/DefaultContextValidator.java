...
public class DefaultContextValidator<C extends ImmutableContext> implements ContextValidator<C> {
    ...
    public void validate(C context) throws ContextValidationException {
        ...
        try {
            ...
            checkNotNull(context.getApplicationId(), "applicationId " + MUST_BE_NOT_NULL);
            checkNotNull(context.getRequestCorrelationId(), "requestCorrelationId " + MUST_BE_NOT_NULL);
            checkNotNull(context.getFunnelCorrelationId(), "funnelCorrelationId " + MUST_BE_NOT_NULL);
            checkNotNull(context.getFunnelName(), "funnelName " + MUST_BE_NOT_NULL);
            checkNotNull(context.getFunnelVersion(), "funnelVersion " + MUST_BE_NOT_NULL);
            checkNotNull(context.getSessionCorrelationId(), "sessionCorrelationId " + MUST_BE_NOT_NULL);
            checkNotNull(context.getIpAddress(), "IpAddress " + MUST_BE_NOT_NULL);
            ...
            if (context.getUserID().isPresent()) {
                ...
                checkArgument(
                        Strings.isNullOrEmpty(context.getUserLang()) || SUPPORTED_LANG.contains(context.getUserLang()),
                        context.getUserLang() + " is not a lang supported by viadeo (fr, es, en, de, pt, it, mx, ru, ar)"
                );

                checkArgument(
                        Strings.isNullOrEmpty(context.getUserCountry()) || countries.contains(context.getUserCountry().toUpperCase()),
                        context.getUserCountry() + " is not a country code ISO 3166 (2 letters)"
                );
            }

        } catch (IllegalArgumentException | NullPointerException e) {
            throw new ContextValidationException("Error to validate the context", e);
        }
    }
}
