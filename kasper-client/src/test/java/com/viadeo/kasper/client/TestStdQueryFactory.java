...
public class TestStdQueryFactory {
    ...
    private ITypeAdapterFactory<Map<String, List<DateTime>>> createTypeAdapterFactory() {
        return new ITypeAdapterFactory<Map<String, List<DateTime>>>() {
            @Override
            public Optional<TypeAdapter<Map<String, List<DateTime>>>> create(TypeToken<Map<String, List<DateTime>>> typeToken, final IQueryFactory adapterFactory) {
                @SuppressWarnings("serial")
                final TypeAdapter<List<DateTime>> dateTimeListAdapter = adapterFactory.create(new TypeToken<List<DateTime>>() {});

                TypeAdapter<Map<String, List<DateTime>>> adapter = new TypeAdapter<Map<String, List<DateTime>>>() {
                    @Override
    ...
    private IQueryFactory createQueryFactory(final Object... queryFactoryParameters) {
        ...
        for (final Object parameter : queryFactoryParameters) {
            if (parameter instanceof TypeAdapter) {
                TypeAdapter<?> adapter = (TypeAdapter<?>) parameter;
                TypeToken<?> adapterForType = TypeToken.of(adapter.getClass()).resolveType(TypeAdapter.class.getTypeParameters()[0]);
