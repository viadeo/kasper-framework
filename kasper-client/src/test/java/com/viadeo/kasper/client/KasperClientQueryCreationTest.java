...
public class KasperClientQueryCreationTest {
    ...
    class MoreComplexQuery implements IQuery {
        ...
        public MoreComplexQuery(final String aStr, final int aPrimitiveInt, final Integer aInteger, 
        		final boolean aPrimitiveBoolean, final Date aDate, final DateTime[] multipleDates) {
            super();
            this.aStr = aStr;
    ...
    public void testComplexPojosWithCustomAdapter() {
        ...
        // When
        final MultivaluedMap<String, String> map = client.prepareQueryParams(new ComplexQuery(new SomePojo("foo bar")));

        // Then
        assertEquals("2", map.getFirst("somePojo"));
        
        // When
        final MultivaluedMap<String, String> map2 = client.prepareQueryParams(new ComplexQuery(new SomePojo("foo bar")));
