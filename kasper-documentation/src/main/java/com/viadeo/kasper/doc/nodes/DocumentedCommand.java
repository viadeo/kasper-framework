...
public final class DocumentedCommand extends AbstractDocumentedDomainNode {
	...
	static public final String TYPE_NAME = "command";
	static public final String PLURAL_TYPE_NAME = "commands";
	...
	public DocumentedCommand(final KasperLibrary kl, final Class<? extends ICommand> commandClazz) {
		...
		if (description.isEmpty()) {
			description = String.format("The %s command", commandClazz.getSimpleName().replaceAll("Command", ""));
		}
	...
	public DocumentedNode getHandler() {
		...
		final Optional<DocumentedHandler> handler = kl.getHandlerForCommand(getDomainName(), getName());
