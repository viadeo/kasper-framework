...
public class HttpQueryExposer extends HttpExposer {
	...
	protected IQuery parseQuery(final String queryName,
		...
		if (queryClass == null) {
			...
			sendError(HttpServletResponse.SC_NOT_FOUND,
                      "No such query[" + queryName + "].", resp, null);

        } else {

			final ITypeAdapter<? extends IQuery> adapter =
                    queryAdapterFactory.create(TypeToken.of(queryClass));

			final Map<String, List<String>> queryParams = new HashMap<>();

            final Enumeration keys = req.getParameterNames();
	...
	private void sendError(final int status, final String message,
