...
public class SpringAMQPTerminal implements EventBusTerminal, InitializingBean, ApplicationContextAware {
    ...
    public void onClusterCreated(final Cluster cluster) {
        ...
        if (clusterMetaData.getProperty(AMQPConsumerConfiguration.AMQP_CONFIG_PROPERTY) instanceof AMQPConsumerConfiguration) {
            config = (AMQPConsumerConfiguration) clusterMetaData.getProperty(AMQPConsumerConfiguration.AMQP_CONFIG_PROPERTY);
        } else {
